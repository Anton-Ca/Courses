...
def student_move(board, student_starts):
    ...
    best_move, score = minimax(
        board, DEPTH, -math.inf, math.inf, column, PLAYER_PIECE, maximize, student_starts)
    if best_move == None:
        print('No more moves')
    else:
...
def score_position(piece, board):
    ...
    center_count = center_array.count(opp_piece)
    score -= center_count * 4 
...
def minimax(board, depth, a, b, column, piece, maximize, maximizingPlayer):
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            ...
            return (None, score_position(maximize, board))
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            ...
            new_score = minimax(board_copy, depth-1, a, b, col, -piece, maximize, False)[1]
            if new_score > value:
                value = new_score
                column = col
            a = max(a, value)
        ...
        for col in valid_locations:
            row = get_next_open_row(board, col)
            board_copy = drop_piece(board, row, col, piece)
            new_score = minimax(board_copy, depth-1, a, b, col, -piece, maximize, True)[1]
            if new_score < value:
                value = new_score
                column = col
            b = min(b, value)
