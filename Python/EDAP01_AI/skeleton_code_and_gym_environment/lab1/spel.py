...
def student_move(board, student_starts):
    ...
    if best_move == None:
        ...
        print('Best move is: ' + str(best_move + 1) + '   With the score: {}'.format(score))
...
def score_position(piece, board):
    ...
    for i in range(board.shape[0]):
        row_array = [int(i) for i in list(board[i, :])]
        for j in range(board.shape[1] - 3):
            window = row_array[j:j+WINDOW_LENGTH]
            score += evaluate_window(window, piece)

    # Score columns on transpose array
    reversed_board = [list(i) for i in zip(*board)]
    for i in range(board.shape[1]):
        col_array = [int(i) for i in list(reversed_board[:, i])]
        for j in range(board.shape[0] - 3):
            window = col_array[j:j+WINDOW_LENGTH]
            score += evaluate_window(window, piece)
    ...
    for i in range(board.shape[0] - 3):
        for j in range(board.shape[1] - 3):
            for k in range(4):
                window = list(reversed_board[i + k][j + k])
                score += evaluate_window(window, piece)
    return score
'''
def score_position(piece, board):
...
def minimax(board, depth, a, b, maximizingPlayer):
    # For alternation of placing down a piece
    valid_locations = get_valid_locations(board)
    is_terminal = is_terminal_node(board)  # Översätt här
    if depth == 0 or is_terminal:
        if is_terminal:
            if env.is_win_state() or winning_move(board, SERVER_PIECE):
                return (None, math.inf)
            elif env.is_win_state() or winning_move(board, PLAYER_PIECE):
                return (None, -math.inf)
            else:
                return(None, 0)
        else:  # Depth is zero
            return (None, score_position(SERVER_PIECE, board))
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            ...
            board_copy = drop_piece(board, row, col, SERVER_PIECE)
            c, new_score = minimax(board_copy, depth-1,
                                   a, b, False)
            new_score = max(value, new_score)
            #print('max: {}'.format(new_score))
            if new_score > value:
                value = new_score
                column = col
            a = max(a, value)
        ...
        for col in valid_locations:
            ...
            c, new_score = minimax(board_copy, depth-1, a, b, True)
            #print('min: {}'.format(new_score))
