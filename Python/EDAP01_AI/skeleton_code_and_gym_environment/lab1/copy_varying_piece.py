...
def student_move(board, student_starts):
    ...
    best_move, score = minimax(board, DEPTH, -math.inf, math.inf, column, student_starts)
    print('Best move is: ' + str(best_move + 1))
#    if env.is_valid_action(col):
#        drop_in_row = env.step(col)
    """
...
def score_position(piece, board):
    ...
    for j in range(2):
        n *= -1
        center_array = [int(i) for i in list(board[:, COLUMN_COUNT//2 + n])]
        center_count = center_array.count(piece)
...
def minimax(board, depth, a, b, column, student_starts):
    if student_starts:
        piece = PLAYER_PIECE
    else:
        piece = AI_PIECE 
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            ...
            return (column, score_position(piece, board))
    ...
    if student_starts:
        ...
        for col in valid_locations:
            ...
            board_copy = drop_piece(board_copy, row, col, piece)
            new_score = max(value, minimax(
                board_copy, depth-1, a, b, col, False)[1])
            if new_score > value:
                value = new_score
                column = col
            a = max(a, new_score)
        ...
        for col in valid_locations:
            ...
            board_copy = board.copy()
            board_copy = drop_piece(board_copy, row, col, -piece)
            new_score = min(value, minimax(board_copy, depth-1, a, b, column, True)[1])
            if new_score > value:
                value = new_score
                column = col
            b = min(b, new_score)
...
def play_game(vs_server=True):
    ...
    while not done:
        ...
        if result != 0:
            ...
            pass#print("Current state (1 are student discs, -1 are servers, 0 is empty): ")
