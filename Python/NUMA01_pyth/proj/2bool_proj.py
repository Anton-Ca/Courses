    ...
    for _ in range(0,1):
        ...
        for ind, _ in enumerate(dates):       # ind, is same as ind, _
            ...
            if abs((current_date - next_date).total_seconds()) >= 240:
                ...
                dist = abs((current_date - next_date).total_seconds())
                while dist > time_step * 60:
                    new_date += timedelta(minutes=time_step)
                    new_count = int((current_count + next_count) / 2)
                    new_elements.append((new_date, new_count))
                    dist -= time_step * 60
                ...
                if new_elements != None:
                    dates[real_ind:real_ind] = [date[0] for date in new_elements]
                    counts[real_ind:real_ind] = [count[1] for count in new_elements]
...
def fix_bio_err(dates, counts):
    ...
    for _ in range(0,2):
        ...
        dates = [d for (d, remove) in zip(dates, R) if not remove]
