...
def fixMissingLine(birdPd):
    ...
    for ind in range(0, birdPd.shape[0]-1):
        if birdPd.loc[ind+1,"DateTime"].minute - birdPd.loc[ind,"DateTime"].minute <= 2 :
            ...
            temp = [birdPd.loc[ind, "DateTime"] + timedelta(minutes=gapMin), birdPd.loc[ind,"Count"]]
            if b == True:
                print(temp)
                b = False
...
def fixMissingLine2(birdPd):
    """
    Implemented method by Harald where
    the situation which several minutes are missed is accounted

    """
    newBird = []
    for ind in range(0, birdPd.shape[0]-1):
        diff = abs(birdPd.loc[ind+1,"DateTime"].minute - birdPd.loc[ind,"DateTime"].minute)
        if diff <= 2 :
            newBird.append([birdPd.loc[ind,"DateTime"], birdPd.loc[ind,"Count"]])
        elif diff > 2 :
            nbr_missing_lines = diff//2
            index = 0
            print(birdPd.loc[ind,"DateTime"])
               temp = [newTime, birdPd.loc[ind,"Count"]]
                print(f"New: {newTime}")
                newBird.append(temp)
    newBird.append([birdPd.iloc[-1].at["DateTime"], birdPd.iloc[-1].at["Count"]]) #last data
