...
def drawNight(birdPd, startDate, dateDelta = 0, prety0 = 'C1', prety='blue', a0=0.3):
    """
    Drawing function, returns a plot which nigh is shaded. 
    There is a few corrupted Boolean for isDayTime. Exact cause need to be investigated.

    """
    start = strDayToInd(birdPd, startDate, 0)
    end = strDayToInd(birdPd, startDate, dateDelta)
    lower = 0
    upper = 0
    dayStart = []
    ...
    for i in tqdm(range(start, end-1)):
        if birdPd.iloc[i].at["isDayTime"] == True and birdPd.iloc[i+1].at["isDayTime"] == False:
            if birdPd.iloc[i+1].at["isDayTime"] == False and birdPd.iloc[i+2].at["isDayTime"] == False:
                lower = i
        if birdPd.iloc[i].at["isDayTime"] == False and birdPd.iloc[i+1].at["isDayTime"] == True:
            if birdPd.iloc[i+1].at["isDayTime"] == True and birdPd.iloc[i+2].at["isDayTime"] == True:
                upper = i-1
    ...
    ax1.set_ylabel('Movement per minute')
    ax1.set_xlabel('Date')
    plt.show()
    return ax1
...
def bargraphs(birdDiff_m): 
    ...
    ax1.set_xlabel('Hour')
    ax1.set_ylabel('Average of movement per minute')
    plt.show()
    ...
    ax2.set_ylabel('Average of movement per minute')
    plt.show()
