...
def resetFix(birdPd):
    ...
    for i in range(birdPd.shape[0]-1):
        ...
        if i ==0:
            continue
        if birdPd.loc[i].at["Count"] < birdPd.loc[i-1].at["Count"] and birdPd.loc[i+1].at["Count"]<birdPd.loc[i-1].at["Count"]:
            j=i
            while birdPd.loc[j].at["Count"] <= birdPd.loc[j+1].at["Count"] and j<(birdPd.shape[0]-2):
                birdPd.loc[j,"Count"]= birdPd.loc[j].at["Count"]+ birdPd.loc[i-1].at["Count"]
...
def dropWeird(birdPd):
    ...
    for i in range(1, birdPd.shape[0]-2):
        ...
        if current < before and before <= after:                # changed
            birdPd.loc[i, "Count"] = float((before+after)/2)
...
def fixMissingLine(birdPd):
    ...
    Its working just with minutes, but we have to reset the hours as well in one place.


    """
    newBird = []
    for i in range(1, birdPd.shape[0]-1):
        if birdPd.loc[i+1,"DateTime"].minute < birdPd.loc[i,"DateTime"].minute:
        ...
        if birdPd.loc[i+1,"DateTime"].minute - birdPd.loc[i,"DateTime"].minute <= 2 :
            ...
            for gapMin in range(birdPd.loc[i,"DateTime"].minute,birdPd.loc[i+1,"DateTime"].minute,2):
...
print("Shape reset Fix", birdPd.shape[0])
