...
inputf = sys.argv[1:]
#print(inputf)
#inputf = ['data/sample/1.in', 'data/secret/0mini.in', 'data/secret/1small.in', 'data/secret/2med.in', 'data/secret/3large.in', 'data/secret/4huge.in']
...
def main():
    for arg in inputf:
        start = timer()
        chars, sim_mat, Q, q = read_data(arg)
        end = timer()
        ...
        # PRINT F-MAT 
        #for i, mat in enumerate(f_mat): print_f_mat(str3[i], str4[i], mat)
...
def read_data(input_file):
    ...
    with open(input_file) as file:
        ...
        k = len(chars)
        for i in range(0,k):
            sim_mat.append(file.readline().split())
        Q = int(file.readline().split()[0])
        for i in range(0, Q):
