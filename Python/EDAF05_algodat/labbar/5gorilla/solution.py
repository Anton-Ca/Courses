...
def main():
    ...
    # Maps letters to indexes in the chars list
...
def read_data2():
    ...
    for i, line in enumerate(sys.stdin):
        ...
        line: str = remove_whitechar(line, "\n+")

        if i < k+1:
            cost_mat.append(line.split())
        if i == k+1:
            Q = int(float(line[0]))
        if i > k+1:
            q.append(line)
...
def remove_whitechar(string: str, expression) -> str:
    remove_whitespace = re.compile(expression)
    new_string = re.sub(remove_whitespace, '', string)
    return new_string
