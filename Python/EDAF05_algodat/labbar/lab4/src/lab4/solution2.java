...
import lab4.run2.Point;

public class solution2 {
	private lab4.run2.Point[] points;

	public solution2(lab4.run2.Point[] points) {
		this.points = points;
	...
	public float findClosestPair(lab4.run2.Point[] sortedPoints) {
		lab4.run2.Point[] sP = sortedPoints;
		int length = sP.length;

		if (length < 4)
			return bruteForce(length).distance();

		lab4.run2.Point[] left = new Point[sP.length / 2];
		lab4.run2.Point[] right = new Point[sP.length / 2];
		...
		Point[] sortedByY = new Point[length]; // Osäker här
	...
	public class Pair {
		...
		public Pair(lab4.run2.Point p1, lab4.run2.Point p2) {
			this.p1 = p1;
			this.p2 = p2;
		}
