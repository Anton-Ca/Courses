...
# Annars rätt med check_solution.sh
# Problemet med tid är på while bfs i edmunds-karp algoritmen, dvs antingen bfs eller hur många gånger vi anropar bfs
...
def edmonds_karp(graph, source, sink, ind):
    ...
    while bfs(graph, source, sink, parent, 0):
        strt = timer()
...
def bfs(graph, s, t, parent, time):
    ...
    time = (end - start)
    #print("TIME TO PERFORM BFS: {} \n".format(timedelta(seconds = time-time)))
    return visited[t]
