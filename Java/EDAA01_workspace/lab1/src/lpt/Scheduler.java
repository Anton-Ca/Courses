...
// Returnerar den maskin som har minst att göra. 
	...
	// (int i = 0, i < machines.length, i++)
	/*
	 * public Machine machineWithLeastToDo() { int min = Integer.MAX_VALUE; int
	 * minPuss = 0; for (Machine i : machines) { int totalTime =
	 * machines[i].getScheduledTime(); if (totalTime < min) { min = totalTime; }
	 * 
	 * minPuss++; } //tempJobList.sort(new DescTimeComp()); /** Fördelar jobben i
	 * listan jobs på maskinerna.
	 */
	...
	/**
	 * Skriver ut maskinernas scheman. public void printSchedule() { for (int i = 0;
	 * i <= machines.length; i++) { System.out.println(machines[i]); } }
	 */
